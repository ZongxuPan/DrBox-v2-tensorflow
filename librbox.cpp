...
void OverlapSub (double *rbox1, double *rbox2, double *area)
{
	...
	area[0] /= 2;
    //cout<<area[0]<<endl;
    area[0] = factor * area[0] / (width1 * height1 + width2 * height2 - area[0]);
...
void Decode(double *preds, double *priors, int n)
{
	for(int i = 0; i < n; i ++)
	{
		...
		prior_angle = prior_angle * 3.141593 / 180;
		//cout << "before:" << rbox_center_x <<rbox_center_y<<endl;
		rbox_center_x *= prior_width;
		rbox_center_y *= prior_height;
		preds[5*i+0] = cos(prior_angle) * rbox_center_x + sin(prior_angle) * rbox_center_y + prior_center_x;
		preds[5*i+1] = sin(prior_angle) * rbox_center_x - cos(prior_angle) * rbox_center_y + prior_center_y;
		preds[5*i+2] = exp(rbox_width) * prior_width;
		preds[5*i+3] = exp(rbox_height) * prior_height;
		//cout << prior_center_x << prior_center_y<<endl;
		//cout << preds[5*i] << preds[5*i+1] <<endl;
		if (rbox_angle > 1)  rbox_angle = 1;
		if (rbox_angle < -1) rbox_angle = -1;
		preds[5*i+4] = (atan(rbox_angle) + prior_angle) * 180 / 3.141593 ;
	}
}
...
extern "C"
{
	...
	float Overlaps(const double *rboxes1, const double *rboxes2)
	{
